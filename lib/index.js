 ...
 */
exports.compile = facets_dsl_1.compile(exports.defaultTerms);
/**
 * compileE compiles to SQL with user supplied values replaced by '?'.
 *
 * The actual values are fed into the provided array.
 */
exports.compileE = function (policies) { return function (options) {
    return function (enabled) { return function (params) { return function (source) {
        return (facets_dsl_1.source2Term({ terms: exports.defaultTerms, policies: policies, options: options })(enabled)(source))
            .chain(function (t) { return t.escape(params); });
    }; }; };
}; };
